...
public class SupabaseRealtimeManager {
    ...
    private void joinChannel() {
        try {
            // Section 2: Subscription verification
            JSONObject joinMsg = new JSONObject();
            joinMsg.put("topic", "realtime:public:messages");
            joinMsg.put("event", "phx_join");
            joinMsg.put("payload", new JSONObject().put("config", new JSONObject().put("postgres_changes", 
            ...
            Log.d(TAG, "Sending phx_join for messages table");
            if (webSocket != null) {
    ...
    private void handleMessage(String text) {
        try {
            ...
            if ("postgres_changes".equals(event)) {
                ...
                // Section 5: Event filtering location
                if (message.getReceiverId() != null && message.getReceiverId().equals(currentUserId)) {
                    Log.d(TAG, "Valid message for current user. Triggering listeners.");
                    for (MessageListener listener : listeners) {
                        listener.onNewMessage(message);
                    }
                } else {
                    Log.d(TAG, "Message filtered out (Receiver mismatch or null)");
                }
